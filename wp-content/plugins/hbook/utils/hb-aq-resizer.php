...
add_action( 'after_setup_theme', 'create_hbook_resize_image_func' );

function create_hbook_resize_image_func() {
	if ( ! function_exists( 'hbook_resize_image' ) ) {
		function hbook_resize_image( $url, $width, $height ) {
			/* WPML Fix */
			...
			$aq_resize = Hb_Aq_Resize::getInstance();
			try {
				$url = $aq_resize->process( $url, $width, $height, true );
				return array(
					'success' => true,
					'url' => $url
				);
