...
class HbAdminPage { 
	...
	protected function display_temporadas($seasons) { ?>
		...
		if ( $seasons ) {			
			...
			foreach ( $seasons as $tmp_id => $tmp_label ) {
				$tmp_id = $tmp_id+1;
				$fields_options .= '<option value="temporada_' . $tmp_id . '">' . $tmp_label['name']. '</option>';
			}
		?>
	...
	protected function display_checkbox_list( $data, $data_type, $display_check_all_box = true, $display_select_all_link = false ) {
		...
		?>
			...
			<label for="hb-checkbox-<?php echo( $data_type ); ?>-<?php echo( $id ); ?>"><?php echo( esc_html( $name ) ); ?></label><br/>
		<?php
		}
			...
			<div class="hb-form-field" data-bind="css: { 'hb-standard-field': standard == 'yes' }, attr: { id: id }">
				...
				<div data-bind="slideVisible: displayed">
					...
					<p data-bind="visible: type() != 'title' && type() != 'sub_title' &&
						...
						['first_name', 'last_name', 'email'].indexOf( id() ) == -1"
					>
